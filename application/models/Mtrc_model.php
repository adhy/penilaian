...
class Mtrc_model extends CI_Model
{
    ...
    // datatables
    function json($id,$di) {
        $this->datatables->select('id_monitoring,id_satker,satker,id_indikator,indikator,id_analisis,analisis,id_tasks,tasks,bulan,rtl_strategi,potential_blocker,pic,tgl_start,tgl_deadline,tgl_tercapai,upload_bukti,catatan_pic,status,stwarna,stket,ara_kasatker,tahun');
       ...
       // $this->datatables->edit_column('upload_bukti',anchor(site_url('arahankasat/download/$1'),'<i class="fa fa-cloud-download" aria-hidden="true"></i>', array('class' => 'btn btn-success btn-sm')), 'upload_bukti, upload_bukti');
        ...
        $this->datatables->add_column('ara_kasatker', '$1', 'ara_kasatker');
